...
# ⚠️ con IMG_SIZE alto, baja batch para no quedarte sin VRAM
IMG_SIZE = 512            # ~500
BATCH_SIZE = 2            # recomendado para 512 (ajusta según tu GPU)
NUM_EPOCHS = 12
...
# =========================
# THRESHOLD CONFIG (para report)
# =========================
# Ya NO lo usamos para decidir el guardado del modelo.
TARGET_RECALL_MAL = 0.95
THRESH_MODE = "max_spec"
THRESH_GRID = 500
...
# FOCAL LOSS CONFIG
# =========================
FOCAL_GAMMA = 1.5  # 1.5–2.0 típico
...
# =========================
# Resolve full dicom path (robusto): prioriza full mammogram
# =========================
...
# =========================
# DICOM loader -> PIL (grayscale) + CROP (quita fondo negro)
# =========================
...
class ResNetBinaryTransfer(nn.Module):
    def __init__(self):
        ...
        w_rgb = self.net.conv1.weight.data.clone()
...
# =========================
# Focal Loss (multi-class) con alpha=class_weights
# =========================
class FocalLoss(nn.Module):
    """
    alpha: tensor shape [2] (pesos de clase)
    gamma: float
    """
...
# =========================
# Threshold utilities (solo para report)
# =========================
...
def find_best_threshold_on_val(
    ...
    for thr in thresholds:
        ...
        score = spec if mode == "max_spec" else (bal if mode == "max_balanced" else f1)
        if score > best_score:
            best_score = score
            best_thr = float(thr)
            best_cm = cm
...
def train():
    ...
    print("Guardando BEST en:", SAVE_PATH)
    print("DEVICE =", DEVICE, "| IMG_SIZE =", IMG_SIZE, "| BATCH_SIZE =", BATCH_SIZE)
    print("Guardado por criterio: VAL AUC (sin usar recall para guardar)")
    ...
    # class weights (para alpha en focal)
    ...
    print(f"FASE 1: solo fc durante {FREEZE_EPOCHS} épocas (lr={LR_HEAD})")

    # ✅ Guardado SOLO por AUC
    best_auc = -1.0
    patience = 5
    bad = 0
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        if improved:
            ...
            torch.save({
                ...
                "img_size": IMG_SIZE,
                "note": "full_mammogram+IMG512+focal_loss+2phase_finetune+best_by_val_auc",
    ...
    # threshold para report (opcional)
    va_loss, _, _, va_auc, _, ys_va, _, p_mal_va = eval_epoch(model, dl_va, loss_fn)
    ...
    print("\n==============================")
    print("THRESHOLD SELECCIONADO EN VAL (solo para report)")
